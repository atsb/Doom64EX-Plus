...
void A_FireLaser(player_t* player, pspdef_t* psp) {
	...
	for (i = 0; i < lasercount; i++) {
		...
		laser[i]->y1 = mobj->y + FixedMul(LASERDISTANCE, finesine[(mobj->angle) >> ANGLETOFINESHIFT]);
		...
		laser[i]->slopex = (finecosine[(angleoffs) >> ANGLETOFINESHIFT] << 5);
		laser[i]->slopey = (finesine[(angleoffs) >> ANGLETOFINESHIFT] << 5);
