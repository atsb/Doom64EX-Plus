...
kexParser *kexlib::parser = &parserLocal;
...
kexLexer *kexParser::Open(const char* filename) {
    ...
    buffsize = kexlib::fileSystem->OpenFile(filename, (byte**)(&buffer), hb_static);

    if(buffsize <= 0) {
        kexlib::system->Warning("kexParser::Open: %s not found\n", filename);
        return NULL;
    }
